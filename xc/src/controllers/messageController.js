...
exports.sendMessage = async (req, res) => {
  ...
  try {
    // Get user settings for message length limit
    const userSettings = await UserSettings.findOne({ user: req.user._id });
    
    // Check message length against user's settings if content is provided
    if (content && userSettings?.enforceLengthLimit) {
      const lengthLimit = userSettings.textMessageLengthLimit || 160; // Default to 160 if not set
      if (content.length > lengthLimit) {
        return res.status(400).json({ 
          message: `Message exceeds your character limit of ${lengthLimit}. Please shorten your message or adjust your settings.`
        });
      }
    }
    ...
    if (file) {
      // Create a consistently formatted URL path for the file
